...
#-----------------------------------------------------------------------------------------------------------------------
# .TimeoutException
timeout = 0.02                                    # Limit timeout (sec)
try:
    response = httpx.get('https://jsonplaceholder.typicode.com/todos/1', timeout=timeout)
    response.raise_for_status()                   # raise - Вызовет исключение при превышении лимита timeout
except httpx.ReadTimeout as e:
    print(f'❌Timeout > {timeout} sec.')          # ❌Timeout > 0.01 sec.
